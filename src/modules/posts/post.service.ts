...
import { TCreatePostPayload, TUpdatePostPayload, IPostDocument, TPostQueryParams } from './post.interface';
...
const getAllPosts = async (query: TPostQueryParams): Promise<{ posts: IPostDocument[], total: number, page: number, limit: number }> => {
    ...
    const posts = await postQuery.modelQuery.populate({ path: 'user', select: 'name email image role' });
